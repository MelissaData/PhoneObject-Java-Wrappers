...
Config_FileName4="mdPhoneJavaWrapper.cpp"
...
########################## Main ############################
printf "\n======================= WELCOME TO MELISSA PHONE OBJECT JAVA WRAPPER =======================\n"
...
# Compile SO
gcc -fpic -c -I$ProjectPath -I$JavaIncludeDirectory1 -I$JavaIncludeDirectory2 mdPhoneJavaWrapper.cpp
gcc -g -shared mdPhoneJavaWrapper.o -o libmdPhoneJavaWrapper.so -L$ProjectPath -lmdPhone -pthread

rm mdPhoneJavaWrapper.o

# Compile Jar
javac com/melissadata/*.java &> /dev/null

jar cvf mdPhone.jar com/melissadata/*.class &> /dev/null

if [ -f "libmdPhoneJavaWrapper.so" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/libmdPhoneJavaWrapper.so"

  printf "\nlibmdPhoneJavaWrapper.so has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create libmdPhoneJavaWrapper.so\n"
fi

if [ -f "mdPhone.jar" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/mdPhone.jar"

  printf "\nmdPhone.jar has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create mdPhone.jar\n"
fi
